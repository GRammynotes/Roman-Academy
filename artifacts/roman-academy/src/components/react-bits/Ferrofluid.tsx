...
const flowVec = (d?: string): [number, number] => {
  switch (d) { case 'up': return [0,1]; case 'left': return [-1,0]; case 'right': return [1,0]; default: return [0,-1]; }
};
...
uniform int uColorCount;uniform vec3 uMouseColor;uniform vec2 uFlow;
...
float vn(vec2 p,float s,float seed){vec2 cellp=floor(p/s);vec2 relp=mod(p,s);float g1=hash(vec3(cellp,seed));float g2=hash(vec3(cellp.x+1.0,cellp.y,seed));float g3=hash(vec3(cellp.x+1.0,cellp.y+1.0,seed));float g4=hash(vec3(cellp.x,cellp.y+1.0,seed));float bx=sinlerp(g1,g2,relp.x/s);float tx=sinlerp(g4,g3,relp.x/s);return sinlerp(bx,tx,relp.y/s);}
...
void mainImage(out vec4 fragColor,in vec2 fragCoord){
  ...
  float distort1=vn(p+perp*(t*spd),60.0,10.0)*50.0*uTurbulence;float distort2=vn(p-perp*(t*spd),120.0,15.0)*100.0*uTurbulence;
...
const Ferrofluid: React.FC<FerrofluidProps> = ({
  className, dpr, paused = false, colors, speed = 0.5, scale = 1.6, turbulence = 1,
  fluidity = 0.1, rimWidth = 0.2, sharpness = 2.5, shimmer = 1.5, glow = 2,
  ...
  useEffect(() => {
    ...
    try {
      renderer = new Renderer({ dpr: dpr ?? window.devicePixelRatio ?? 1, alpha: true, antialias: true });
      ...
      return () => {
        ...
        rendererRef.current = null; programRef.current = null; geometryRef.current = null; meshRef.current = null;
      };
    } catch {
      return () => {
        ro?.disconnect();
  ...
  }, [dpr, paused, JSON.stringify(colors), speed, scale, turbulence, fluidity, rimWidth, sharpness, shimmer, glow, flowDirection, opacity, mouseInteraction, mouseStrength, mouseRadius, mouseDampening]);
