...
const TEACHERS = [
  { name: "Abhijeet Roman", role: "Core subjects & Mentorship", exp: "10–12+ years experience", expertise: "PCB, Commerce core, Languages", title: "Civil Engineer" },
  { name: "Navnath Roman", role: "Math & Physics Expert", exp: "15 years teaching experience", expertise: "Maths, Organic Chem, Electronics", title: "Electronic Engineer" },
  { name: "Kunal Datkhile", role: "PCM & Aptitude", exp: "2 years teaching experience", expertise: "Cyber security & AI enthusiast", title: "Computer Engineer • 97%ile CET" },
  { name: "Rinki Yadav", role: "Biology & Chemistry", exp: "4 years teaching experience", expertise: "NEET foundation", title: "500+ NEET score" },
];
    ...
    <div className="min-h-screen bg-navy-950 scroll-smooth">
      ...
      <section id="batches" className="px-4 py-20 bg-navy-900/50 border-y border-gold-400/5">
        <div className="max-w-6xl mx-auto space-y-12">
          ...
          <div className="grid md:grid-cols-2 gap-6">
            <div className="relative rounded-2xl border border-gold-400/20 bg-navy-950 overflow-hidden shadow-xl p-8 space-y-6">
              <div className="absolute top-4 right-4">
                <span className="bg-emerald-500/10 text-emerald-400 border border-emerald-500/20 text-xs font-bold px-2 py-1 rounded-md">Filling Fast</span>
              </div>
              <div>
            ...
            <div className="relative rounded-2xl border border-gold-400/20 bg-navy-950 overflow-hidden shadow-xl p-8 space-y-6">
              <div className="absolute top-4 right-4">
                <span className="bg-amber-500/10 text-amber-400 border border-amber-500/20 text-xs font-bold px-2 py-1 rounded-md">Active</span>
              </div>
              <div>
      ...
      <section id="faculty" className="px-4 py-20 max-w-6xl mx-auto space-y-12">
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-4">
          {TEACHERS.map((teacher) => (
            <div key={teacher.name} className="flex flex-col bg-white/5 border border-gold-400/15 rounded-2xl overflow-hidden hover:border-gold-400/40 transition-colors p-5 space-y-4">
        ...
        <div className="max-w-6xl mx-auto space-y-12">
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 gap-3 md:gap-4">
            {GALLERY_ITEMS.map((item) => (
              <div key={item.id} onClick={() => setActiveGalleryItem(item)} className="group relative aspect-video bg-navy-950 rounded-xl overflow-hidden cursor-pointer border border-gold-400/10 hover:border-gold-400/40 transition-all">
      ...
      {activeGalleryItem && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/95 p-4 backdrop-blur-md">
          <button onClick={() => setActiveGalleryItem(null)} className="absolute top-6 right-6 p-2 bg-white/10 hover:bg-white/20 rounded-full text-white transition-colors">
            <X className="size-6" />
          </button>
          <div className="w-full max-w-5xl">
            <img src={activeGalleryItem.src} alt={activeGalleryItem.label} className="w-full h-auto max-h-[80vh] object-contain rounded-lg" />
          ...
          <div className="bg-navy-950 border border-gold-400/30 rounded-2xl shadow-2xl max-w-md w-full overflow-hidden">
            ...
            <div className="p-6 space-y-6">
              ...
              <div className="flex flex-col gap-3">
                <Link href="/contact?reason=Admission+Enquiry" onClick={() => setShowPopup(false)} className="block text-center py-3 bg-gold-400 text-navy-950 font-bold rounded-xl hover:bg-gold-300 transition-colors">
                  Enquire About Admissions
                </Link>
                <button onClick={() => setShowPopup(false)} className="text-sm text-ivory-100/60 hover:text-ivory-100/90 transition-colors">
