...
router.get("/admin/archived-students", requireRole(["teacher", "admin"]), async (req: any, res) => {
  try {
    const students = await db.select({
      ...
      .leftJoin(usersTable, eq(studentsTable.userId, usersTable.id))
      .where(eq(studentsTable.archived, true));

    return res.json(students);
  } catch (err) {
    logger.error({ err }, "Admin archived students error");
    return res.status(500).json({ error: "Internal server error" });
  }
});
...
router.patch("/admin/students/:id/restore", requireRole(["teacher", "admin"]), async (req: any, res) => {
  try {
    ...
    const existing = await db.select({ id: studentsTable.id })
      .from(studentsTable)
      .where(and(eq(studentsTable.id, id), eq(studentsTable.archived, true)))
      .limit(1);
    if (!existing.length) return res.status(404).json({ error: "Archived student not found" });
    await db.update(studentsTable).set({ archived: false }).where(eq(studentsTable.id, id));
    return res.json({ success: true });
  } catch (err) {
    logger.error({ err }, "Admin restore student error");
    return res.status(500).json({ error: "Internal server error" });
  }
});
